...
iomux_v3_cfg_t const usb_pads[] = {
	...
	MX6_PAD_KEY_COL4__USB_OTG_OC    | MUX_PAD_CTRL(GPIO_PAD_CTRL),   // USB_OTG_OC
};
...
int board_ehci_hcd_init(int port)
{
	...
	imx_iomux_v3_setup_multiple_pads(usb_pads, ARRAY_SIZE(usb_pads));
        ...
        bool val = gpio_direction_output(IMX_GPIO_NR(4, 15), 1);

        printf("gpio_direction_output returned : %d\n", val);


        val = gpio_set_value(IMX_GPIO_NR(4, 15), 1);
        printf("gpio_set_value returned : %d\n", val);
...
int board_mmc_init(bd_t *bis)
{
	...
	status = fsl_esdhc_initialize(bis, &usdhc_cfg[0]) | fsl_esdhc_initialize(bis, &usdhc_cfg[1]);
...
int board_init(void)
{
	...
	u32 fuse_value = 0;
	int rva = 0;
	rva = fuse_read(0, 6, &fuse_value);
	if (rva != 0) {
		printf("Failed to read fuse bank 0 6\n");
	}
	else if ((fuse_value & 0x00c00000) != 0x00c00000) {
		printf("Fuse value was %u\n", fuse_value);
		printf("Fuse bank 0 6 was not 0x0000c000, blowing fuses.\n");
		rva = fuse_prog(0, 6, 0x00c00000);

		if (rva != 0) {
			printf("Blowing the fuses FAILED.\n");
		}
	}
	else {
		printf("Read %u\n", fuse_value);
